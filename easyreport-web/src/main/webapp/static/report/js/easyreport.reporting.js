...
var reporting = {
	...
	init : function() {
		...
		$('#queryParamFormElement').change(function() {
			...
			if (formElement == "select" || formElement == "selectMul") {
				$("#queryParamSqlText").removeAttr("disabled");
				$("#queryParamOptionType").val("0");
			} else {
				$("#queryParamSqlText").val('');
				$("#queryParamSqlText").attr({
					"disabled" : "disabled"
				});
				$("#queryParamOptionType").val("2");
			}
		});
		...
		$('#reportingDsId').combobox({
			url : XFrame.getContextPath() + '/datasource/getall',
			valueField : 'id',
			textField : 'name',
			editable : false
		});
	...
	tabs : {
		...
		tabSelectedHandlder : function(title, currIndex) {
		...
		contextMenuOnClick : function(item) {
		...
		resetReportingTabForm : function() {
			$("#reportingDsId").val("");
			$("#reportingLayoutType").val("1");
	...
	tree : {
		...
		saveNode : function() {
			...
			var actUrl = act == "add" ? "addtreenode" : "edittreenode";
			$('#setTreeNodeForm').form('submit', {
				...
				success : function(data) {
					...
					if (result.success) {
						Reporting.updateTreeNode(act, result.jsonObject);
						$('#setTreeNodeDlg').dialog('close');
					}
		...
		nodeOnSelect : function(node) {
			$('#reportingTree').tree('options').url = 'listchildnodes';
		...
		nodeOnDrop : function(target, source, point) {
			var targetNode = $('#reportingTree').tree('getNode', target);
			if (targetNode) {
				$.post(XFrame.getContextPath() + '/reporting/dragtreenode', {
					sourceId : source.id,
		...
		pasteNode : function() {
			var node = Reporting.getSelectedTreeNode();
			if (node) {
				$.post(XFrame.getContextPath() + '/reporting/pastetreenode', {
					sourceId : $('#copyNodeId').val(),
					targetId : node.id
				}, function(result) {
					if (result.success) {
						$('#copyNodeId').val(0);
						return Reporting.updateTreeNode('add', result.jsonObject);
					}
		...
		selectAndExpandNode : function(id) {
			var node = $('#reportingTree').tree('find', id);
			if (node) {
				$('#reportingTree').tree('select', node.target);
				Reporting.selectTreeNodeHandler(node);
		...
		updateNode : function(act, nodeData) {
			if (nodeData == null) {
				return;
			}
			if (act == "add") {
	...
	sqlEditors : {
		setExceSqlCmEditorStatus : function() {
		...
		viewSqlText : function() {
			...
			$.post(XFrame.getContextPath() + '/reporting/viewsqltext', {
				dsId : dsId,
				sqlText : sqlText,
				dataRange : dataRange,
				...
				if (result.success) {
					...
					return viewSqlCmEditor.setValue(result.jsonObject);
				}
				return $.messager.alert('错误', result.msg);
			}, 'json');
		},
		viewSqlHistory : function() {
			...
			var url = XFrame.getContextPath() + '/reporting/getallhistorysql?reportId=' + reportId;
			ReportCommon.loadDataToGrid('#viewSqlTextHistoryGrid', url);
			viewSqlHistoryCmEditor.setValue("");
			viewSqlHistoryCmEditor.refresh();
		},
		showHistorySqlText : function(index, row) {
	...
	dailogs : {
		searchReportDlg : {
			...
			search : function() {
				...
				var url = XFrame.getContextPath() + '/reporting/search?fieldName=' + fieldName + '&keyword=' + keyword;
				return ReportCommon.loadDataToGrid('#searchReportGrid', url);
			},
			gridRowOnDblClick : function(index, row) {
	...
	settings : {
		...
		edit : function(data) {
			...
			$("#sqlColumnGrid").datagrid('loadData', eval(data.jsonColumns));
		...
		executeSql : function() {
			...
			var queryParamRows = $("#queryParamGrid").datagrid('getRows');
			var jsonQueryParams = queryParamRows ? JSON.stringify(queryParamRows) : "";
			$.post(XFrame.getContextPath() + '/reporting/loadsqlcolumns', {
				sqlText : sqlText,
				...
				if (result.success) {
					...
					return Reporting.loadSqlColumns(result.jsonObject);
				}
				return $.messager.alert('错误', result.msg);
			}, 'json');
		},
		save : function() {
			...
			$.post(XFrame.getContextPath() + '/reporting/' + act, {
				id : reportId,
				...
				layoutType : $("#reportingLayoutType").val(),
				sqlText : exceSqlCmEditor.getValue(),
				jsonColumns : JSON.stringify(sqlColumnRows),
				...
				$.messager.progress("close");
				if (data.success) {
					Reporting.updateTreeNode(act, data.jsonObject);
					$('#reportingIsChange').val(0);
				}
				$.messager.alert('操作提示', data.msg, data.success ? 'info' : 'error');
			}, 'json');
		},
		preview : function() {
			var node = Reporting.getSelectedTreeNode();
			if (node.attributes.flag == 1) {
				...
				var previewUrl = XFrame.getContextPath() + "/reporting/uid/" + node.attributes.uid;
				$('#tabs').tabs(
						'add',
						{
							id : node.id,
		...
		sqlColumn : {
			...
			resortRows : function(index, type, grid) {
			...
			addRow : function() {
				...
				$.post(XFrame.getContextPath() + '/reporting/getsqlcolumn', function(row) {
			...
			deleteRow : function() {
				...
				if (row && row.type == 4) {
					...
					$("#sqlColumnGrid").datagrid('reload', $("#sqlColumnGrid").datagrid('getRows'));
				} else {
					$.messager.alert('失败', "您不能删除非计算列！", 'error');
				}
			},
			expressionSettings : function(id) {
			...
			getEmptyExprColumns : function(sqlColumnRows) {
				var emptyColumns = [];
				for (var i = 0; i < sqlColumnRows.length; i++) {
	...
	queryParams : {
		loadData : function(data) {
			$("#jsonQueryParams").val(data.jsonQueryParams);
		...
		save : function() {
			...
			$.post(XFrame.getContextPath() + '/reporting/setqueryparam', {
				id : id,
				jsonQueryParams : $('#jsonQueryParams').val()
			}, function(data) {
				$.messager.progress("close");
				if (data.success) {
					Reporting.updateTreeNode('edit', data.jsonObject);
				}
				$.messager.alert('操作提示', data.msg, data.success ? 'info' : 'error');
			}, 'json');
		},
		...
		edit : function(index, row) {
			...
			$("#queryParamOptionType").val(row.optionType ? row.optionType : 2);
			if (row.formElement == "select" || row.formElement == "selectMul") {
				$("#queryParamSqlText").removeAttr("disabled");
				$("#queryParamSqlText").val(row.sqlText);
				return;
			}
			$("#queryParamSqlText").attr({
				"disabled" : "disabled"
	...
	properties : {
		...
		getPropertyValue : function(name, object) {
			...
			if (name == "layoutType") {
				return Reporting.getLayoutTypeName(value);
			}
			if (name == "hasChild") {
				return value ? "有" : "无";
			}
	...
	formatters : {
		queryParamGrid : {
			optionFormatter : function(value, row, index) {
				...
				return '<a href="#" title ="删除" onclick="javascript:Reporting.deleteQueryParam(' + index
						+ ')"><img src="' + path + 'remove.png" alt="删除"/"></a>';
			}
		},
		sqlColumnGrid : {
			optionsFormatter : function(value, row, index) {
				var subOptions = [];
				if (row.type == 4) {
					subOptions = $.grep(configOptions, function(option, i) {
				...
				for (var i = 0; i < subOptions.length; i++) {
					...
					if (name == "expression") {
						...
						html = "<img style=\"cursor: pointer;\" id=\"" + id + "\" title=\"" + text + "\" src=\""
								+ imgSrc + "\"" + onClick + "/>";
					} else {
			...
			sqlColumnNameFormatter : function(value, row, index) {
				var id = "name" + index;
				return "<input style=\"width:98%;\" type=\"text\" id=\"" + id + "\" name=\"name\" value=\"" + row.name
						+ "\" />";
			},
			decimalsFormatter : function(value, row, index) {
				var id = "decimals" + index;
				if (!row.decimals) {
					row.decimals = 0;
				}
				return "<input style=\"width:42px;\" type=\"text\" id=\"" + id + "\" name=\"text\" value=\""
						+ row.decimals + "\" />";
			},
			expressionFormatter : function(value, row, index) {
				if (!row.expression) {
					row.expression = '';
				}
				return row.expression;
			},
			layoutTypeFormatter : function(value, row, index) {
				return Reporting.getLayoutTypeName(row.layoutType);
			},
			...
			sortTypeFormatter : function(value, row, index) {
				var options = [ {
					text : "默认",
					value : 0
				}, {
